...
Ns=1
nu=0.01
model = torch.load('./Result/1000.pth')
model = model.cuda()
...
for i in range(len(ParaList)):
	...
	outputV=output_pad[:,0,:,:].reshape(output_pad.shape[0],1,
										...
										output_pad.shape[3])
	#Impose BC
	outputV[0,0,-padSingleSide:,padSingleSide:-padSingleSide]=(outputV[0,0,1:2,padSingleSide:-padSingleSide]) # up outlet bc zero gradient 
	outputV[0,0,:padSingleSide,padSingleSide:-padSingleSide]=(outputV[0,0,-2:-1,padSingleSide:-padSingleSide])	# down inlet bc
