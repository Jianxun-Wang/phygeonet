...
def train(epoch):
	...
	for iteration, batch in enumerate(training_data_loader):
		...
		for j in range(batchSize):
			#Impose BC
			outputV[j,0,-padSingleSide:,padSingleSide:-padSingleSide]=(outputV[j,0,1:2,padSingleSide:-padSingleSide]) 
			outputV[j,0,:padSingleSide,padSingleSide:-padSingleSide]=(outputV[j,0,-2:-1,padSingleSide:-padSingleSide])	
		...
		if epoch%1000==0 or epoch%nEpochs==0:
			...
			for j in range(batchSize):
				...
				fig1.savefig('Epoch'+str(epoch)+'Para'+str(j)+'T.pdf',bbox_inches='tight')
				#plt.show()
				plt.close(fig1)
